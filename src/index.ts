...
export const handler = async (data: any, context: any) => {
  ...
  if (data.name && typeof data.name === "string") {
    console.debug(
      `A name of length ${data.name.length} has arrived into the Function.`
    );
    ...
    return {
      message: `Hello from a Function! It seems you have ${data.name.length} letters in your name. ${otherStr}`,
    ...
    return {
      message:
        "Hello from a Function! Send an encrypted `name` parameter to show Function decryption in action",
    };
  }
};
